...
export function PageHeader({ menu, settingsMenu, search, onSearchChange, onCreateAsset, currentUserRole }: PageHeaderProps) {
  ...
  return (
    <div className="rounded-[14px] border border-slate-700 bg-slate-800 p-3 shadow-[0_16px_34px_rgba(15,23,42,0.22)]">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h2 className="mt-1 text-lg font-semibold tracking-tight text-white">{getPageTitle(menu, settingsMenu)}</h2>
          <p className="mt-2 text-xs text-slate-200">{getPageDescription(menu, settingsMenu)}</p>
        </div>
        ...
        {showSearch && (
          <div className="flex flex-col gap-3 sm:flex-row">
            <div className="relative min-w-[280px]">
              ...
              <Input
                className="h-11 rounded-[10px] border-white/20 bg-white pl-10"
            ...
            {(menu === "hardware" || menu === "software") && canManageAssets(currentUserRole) && (
              <Button className="h-11 rounded-[10px] px-5" onClick={onCreateAsset}>
